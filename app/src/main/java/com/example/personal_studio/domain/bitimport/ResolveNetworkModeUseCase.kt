...
/**
 * 决定本次同步的**首选**网络模式(校内优先)。
 *
 * - `lastMode = LOCAL/null` → 直接 LOCAL(校内优先,无需探测;校外不可达会由 [autoNetworkFallback]
 *   等回退自动转 WEBVPN)。
 * - `lastMode = WEBVPN` → 先用 [CampusReachabilityProbe] 快速探测校内可达性:可达(回到校园网)→ LOCAL
 *   (自动脱离「粘在校外」);不可达(仍在校外)→ WEBVPN(仅多花一次 ~2s 探测)。
 *
 * 解决「webvpn 在校园网下也能绕一圈成功 → lastMode 一直停在 WEBVPN、永远走慢路」:只在已记住
 * WEBVPN 时才探一次校内。其余情况零探测开销。
 */
